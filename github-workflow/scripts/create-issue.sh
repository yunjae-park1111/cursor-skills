...
if [ -n "$EPIC_NUMBER" ]; then
  EPIC_TARGET="${EPIC_REPO:-$CURRENT_REPO}"

  if [ "$EPIC_TARGET" = "$CURRENT_REPO" ]; then
    gh sub-issue add "$EPIC_NUMBER" "$ISSUE_NUMBER" 2>/dev/null || true
  else
    EPIC_URL="https://github.com/$OWNER/$EPIC_TARGET/issues/$EPIC_NUMBER"
...
if [ -n "$PROJECT_NUMBER" ]; then
  ...
  set_select "Estimate" "$ESTIMATE"

  # Sprint: 최신 iteration 자동 설정
  SPRINT_FIELD_ID=$(echo "$FIELDS" | jq -r '.fields[] | select(.type == "ProjectV2IterationField") | .id')
  SPRINT_ITERATION_ID=$(gh project item-list "$PROJECT_NUMBER" --owner "$OWNER" --format json --limit 50 2>/dev/null | \
